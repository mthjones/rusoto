                    ...
                    pub fn from_body(body: &str) -> DescribeRuleError {
                        match from_str::<SerdeJsonValue>(body) {
                            Ok(json) => {
                                ...
                                match *error_type {
                                    "ResourceNotFoundException" => DescribeRuleError::ResourceNotFound(String::from(error_message)),"InternalException" => DescribeRuleError::Internal(String::from(error_message)),"ValidationException" => DescribeRuleError::Validation(error_message.to_string()),_ => DescribeRuleError::Unknown(String::from(body))
                                }
                            },
...
Unknown(String)
                ...
                impl Error for TestEventPatternError {
                    fn description(&self) -> &str {
                        match *self {
                            TestEventPatternError::InvalidEventPattern(ref cause) => cause,TestEventPatternError::Internal(ref cause) => cause,TestEventPatternError::Validation(ref cause) => cause,TestEventPatternError::Credentials(ref err) => err.description(),TestEventPatternError::HttpDispatch(ref dispatch_error) => dispatch_error.description(),TestEventPatternError::Unknown(ref cause) => cause
                        }
                    }
                 }
/// A client for the Amazon CloudWatch Events API.
        ...
        impl<P, D> CloudWatchEventsClient<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
         ...
         request.set_payload(Some(encoded.into_bytes()));
                ...
                pub fn put_events(&self, input: &PutEventsRequest)  -> Result<PutEventsResponse, PutEventsError> {
                    let mut request = SignedRequest::new("POST", "events", self.region, "/");
